...
use io_engine_api::v0::{json_rpc_server::JsonRpc, JsonRpcReply, JsonRpcRequest};
...
impl JsonRpc for JsonRpcSvc {
    ...
    async fn json_rpc_call(&self, request: Request<JsonRpcRequest>) -> GrpcResult<JsonRpcReply> {
...
impl JsonRpcSvc {
    async fn spdk_jsonrpc_call(&self, method: &str, arg: Option<&str>) -> Result<String, Error> {
        let params: Option<serde_json::Value> = arg.map(serde_json::from_str).transpose()?;

        let result: serde_json::Value = jsonrpc::call(&self.rpc_addr, method, params).await?;
