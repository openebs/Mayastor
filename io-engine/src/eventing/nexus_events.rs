use events_api::event::{
    EventAction, EventCategory, EventMessage, EventMeta, EventSource, RebuildStatus,
};
...
impl EventMetaGen for NexusRebuildJob {
    fn meta(&self) -> EventMeta {
        let rebuild_status = match self.state() {
            RebuildState::Init | RebuildState::Running => RebuildStatus::Started,
...
pub(crate) fn state_change_event_meta(previous: NexusState, next: NexusState) -> EventMeta {
    let event_source = EventSource::new(MayastorEnvironment::global_or_default().node_name)
...
pub(crate) fn subsystem_pause_event_meta(
    ...
    if let Some(total_time) = total_time_option {
        event_source = event_source.with_event_action_duration_details(total_time);
    }
