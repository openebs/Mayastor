...
async fn create_nexus(nexus_size: u64, child_sizes: Vec<u64>) -> bool {
    ...
    if let Err(error) = nexus_create("core_nexus", nexus_size * 1024 * 1024, None, &children).await
    {
...
async fn child_size_ok() {
    mayastor()
        .spawn(async {
            ...
            let bdev = UntypedBdev::lookup_by_name("m0").expect("child bdev m0 not found");
            assert_eq!(bdev.name(), "m0");

            let bdev = UntypedBdev::lookup_by_name("m1").expect("child bdev m1 not found");
            assert_eq!(bdev.name(), "m1");

            let bdev = UntypedBdev::lookup_by_name("m2").expect("child bdev m2 not found");
            assert_eq!(bdev.name(), "m2");

            let nexus = nexus_lookup_mut("core_nexus").expect("nexus not found");
