...
const PVREMOVE_COMMAND: &str = "pvremove";
const VGCHANGE_COMMAND: &str = "vgchange";
const VGS_COMMAND: &str = "vgs";
const VGREMOVE_COMMAND: &str = "vgremove";
...
pub struct VolGroup {
    ...
    vg_name: String,
    /// corresponds to the vg_size field in json output
    #[serde(deserialize_with = "deserialize_number_from_string")]
    vg_size: u64,
    /// corresponds to the vg_free field in json output
    #[serde(deserialize_with = "deserialize_number_from_string")]
    vg_free: u64,
    #[serde(skip_deserializing)]
    disks: Vec<String>, /* /// corresponds to the pv_count field in json
                         * output, the number of
                         * /// physical volumes in the vol group
                         * pv_count: u64,
                         * /// corresponds to the lv_count field in json
                         * output, the number of
                         * /// logical volumes created on the vol group
                         * lv_count: u64 */
}
...
impl VolGroup {
    ...
    pub async fn remove(self) -> Result<(), Error> {
        let disks = self.disks.as_slice();
        let name = self.vg_name.as_str();
        let output = Command::new(VGREMOVE_COMMAND).arg(name).output().await?;
        if !output.status.success() {
            let msg = String::from_utf8(output.stderr).map_or_else(
                |e: std::string::FromUtf8Error| {
                    format!(
                        "failed to parse stderr for vg_remove: {}",
                        e.to_string()
                    )
                },
                |s| s,
        ...
        let output =
            Command::new(PVREMOVE_COMMAND).args(disks).output().await?;
        if !output.status.success() {
            let msg = String::from_utf8(output.stderr).map_or_else(
                |e: std::string::FromUtf8Error| {
                    format!(
                        "failed to parse stderr for pv_remove: {}",
                        e.to_string()
                    )
                },
                |s| s,
